...
var ins = {
  ...
  init : () => {
    ...
    ins.hForm = document.getElementById("EOform");
    

    // (A2) ATTACH CONTROLS
    
    ins.hForm.onsubmit = ins.save;
  ...
  // (F) SAVE EVENT
  save : () => {
    ins.ajax(new FormData(ins.hForm), (res) => {
      if (res=="OK") { console.log("5")}
      else { alert(res); }
    });
    return false;
  },
...
window.addEventListener("DOMContentLoaded", ins.init);
