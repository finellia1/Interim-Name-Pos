...
class Calendar {
  ...
  function __construct () {
    try {
      ...
      $password = "";
      $this->pdo = new PDO(
        "mysql:host=localhost;dbname=hv_audio_visual" , $username, $password, [
  ...
  function save ($type, $is_nonprofit, $order_date, $start, $end, $setup, $breakdown, $is_staffed, $creator, $name, $attendees, $contact, $notes, $id=null) {
    ...
    // INSERT OR UPDATE
    if ($id==null) {
      $sql = "INSERT INTO `event_order` (`event_type`, `is_nonprofit`, `order_date`, `event_start`, `event_end`, `setup_start`, `breakdown_start`, `is_staffed`, `created_by`, `event_name`, 
