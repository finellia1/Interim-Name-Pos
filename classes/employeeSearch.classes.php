    ...
    protected function getEmployees($searchType, $searchContent) {
        ...
        foreach($getData as $row){
            if($row['is_inactive'] == 0){
                ...
                printf('<td><button type="button" onclick="editPane_client(%s,%s, %s,%s,%s,%s,%s,%s,%s)">Edit</button>',$p_id,$p_security_type,$p_pwd,$p_job_title,$p_first_name, $p_last_name, $p_email, $p_hourly_salary,$p_yearly_salary);
                ...
                echo "<td> <input type='hidden' name='PID' id='deleteID' value='{$row['employee_ID']}'> </td>";
                echo "</form>";
                echo "</tr>";
            }
        }
